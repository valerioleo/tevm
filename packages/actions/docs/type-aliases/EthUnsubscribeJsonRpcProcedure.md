[**@tevm/actions**](../README.md)

***

[@tevm/actions](../globals.md) / EthUnsubscribeJsonRpcProcedure

# Type Alias: EthUnsubscribeJsonRpcProcedure

> **EthUnsubscribeJsonRpcProcedure** = (`request`) => `Promise`\<[`EthUnsubscribeJsonRpcResponse`](EthUnsubscribeJsonRpcResponse.md)\>

Defined in: [packages/actions/src/eth/EthProcedure.ts:249](https://github.com/evmts/tevm/blob/main/packages/actions/src/eth/EthProcedure.ts#L249)

## Parameters

| Parameter | Type |
| ------ | ------ |
| `request` | [`EthUnsubscribeJsonRpcRequest`](EthUnsubscribeJsonRpcRequest.md) |

## Returns

`Promise`\<[`EthUnsubscribeJsonRpcResponse`](EthUnsubscribeJsonRpcResponse.md)\>
