[**@tevm/actions**](../README.md)

***

[@tevm/actions](../globals.md) / AnvilSetBlockGasLimitProcedure

# Type Alias: AnvilSetBlockGasLimitProcedure

> **AnvilSetBlockGasLimitProcedure** = (`request`) => `Promise`\<[`AnvilSetBlockGasLimitJsonRpcResponse`](AnvilSetBlockGasLimitJsonRpcResponse.md)\>

Defined in: [packages/actions/src/anvil/AnvilProcedure.ts:243](https://github.com/evmts/tevm/blob/main/packages/actions/src/anvil/AnvilProcedure.ts#L243)

JSON-RPC procedure for `anvil_setBlockGasLimit`

## Parameters

| Parameter | Type |
| ------ | ------ |
| `request` | [`AnvilSetBlockGasLimitJsonRpcRequest`](AnvilSetBlockGasLimitJsonRpcRequest.md) |

## Returns

`Promise`\<[`AnvilSetBlockGasLimitJsonRpcResponse`](AnvilSetBlockGasLimitJsonRpcResponse.md)\>
