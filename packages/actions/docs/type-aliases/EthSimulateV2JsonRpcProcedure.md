[**@tevm/actions**](../README.md)

***

[@tevm/actions](../globals.md) / EthSimulateV2JsonRpcProcedure

# Type Alias: EthSimulateV2JsonRpcProcedure

> **EthSimulateV2JsonRpcProcedure** = (`request`) => `Promise`\<[`EthSimulateV2JsonRpcResponse`](EthSimulateV2JsonRpcResponse.md)\>

Defined in: [packages/actions/src/eth/EthProcedure.ts:263](https://github.com/evmts/tevm/blob/main/packages/actions/src/eth/EthProcedure.ts#L263)

## Parameters

| Parameter | Type |
| ------ | ------ |
| `request` | [`EthSimulateV2JsonRpcRequest`](EthSimulateV2JsonRpcRequest.md) |

## Returns

`Promise`\<[`EthSimulateV2JsonRpcResponse`](EthSimulateV2JsonRpcResponse.md)\>
