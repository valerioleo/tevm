[**@tevm/actions**](../README.md)

***

[@tevm/actions](../globals.md) / DebugStorageRangeAtHandler

# Type Alias: DebugStorageRangeAtHandler

> **DebugStorageRangeAtHandler** = (`params`) => `Promise`\<[`DebugStorageRangeAtResult`](DebugStorageRangeAtResult.md)\>

Defined in: [packages/actions/src/debug/DebugHandler.ts:58](https://github.com/evmts/tevm/blob/main/packages/actions/src/debug/DebugHandler.ts#L58)

## Parameters

| Parameter | Type |
| ------ | ------ |
| `params` | [`DebugStorageRangeAtParams`](DebugStorageRangeAtParams.md) |

## Returns

`Promise`\<[`DebugStorageRangeAtResult`](DebugStorageRangeAtResult.md)\>
