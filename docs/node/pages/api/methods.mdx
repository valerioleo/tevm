...
const result = await vm.runTx({ tx, skipNonce: true, skipBalance: true })
...
import { bytesToHex, createMemoryClient } from 'tevm'
...
const result = await client.tevmCall({
  ...
  onAfterMessage: (result, next) => {
    console.log(`Return: ${bytesToHex(result.execResult.returnValue)}`)
    next?.()
  }
})
```
...
console.log('Deployed:', deployResult.createdAddress)
console.log('Gas used:', deployResult.executionGasUsed)
```
...
`await node.deepCopy()` creates an independent copy of the node's blockchain,
state manager, txpool, and receipt data. State copying is useful for alternative
execution paths and expensive test setup. Use the higher-level
`client.snapshot()` and `client.revert({ id })` actions when a test only needs
