...
:::warning[Arbitrum custom transaction types]
Optimism deposit transactions (type `0x7e`) are reconstructed with exact block
fidelity. Arbitrum transaction types `0x6a`–`0x6f` are not implemented yet;
Tevm throws `UnsupportedForkTransactionError` rather than constructing a block
whose transactions differ from upstream.
:::
