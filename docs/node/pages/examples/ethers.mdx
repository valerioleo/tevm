...
Create a memory client and pass its EIP-1193 provider interface directly to ethers.
...
await client.setBalance({ address: getAddress(signer.address), value: parseEther('10') })
...
console.log(`Initial count: ${await counter.getFunction('number')()}`)
```
...
console.log(`Updated count: ${await counter.getFunction('number')()}`)
```
...
const provider = new BrowserProvider(client, undefined, { cacheTimeout: -1 })
