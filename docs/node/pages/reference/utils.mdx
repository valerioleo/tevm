...
Lightweight utility functions and classes built on [`tevm/utils`](https://github.com/evmts/tevm/tree/main/packages/utils), [`@evmts/zevm`](https://github.com/evmts/zevm), and [`viem`](https://viem.sh).
...
Lower-level helpers from [`tevm/utils`](https://github.com/evmts/tevm/tree/main/packages/utils), ZEVM, or [`viem`](https://viem.sh):
...
Full API: [packages/utils/docs](https://github.com/evmts/tevm/tree/main/packages/utils/docs). Combines ZEVM primitives, viem utilities, and Tevm helpers.

## Installation

```bash
npm install @tevm/utils@1.0.0-rc.151 viem
```
...
```typescript
import { keccak256, ecrecover, randomBytes } from '@tevm/utils'
...
invariant(condition, new Error('Error message'))
```
